...
// writeNeighborhoods = fs.createWriteStream('BACK-END/seeding/cassandraSeed/test.csv');
// writeNeighborhoods.write('listing_id,neighborhood_id,name,dog_friendly,grocery_stores,neighbors_friendly,parking_easy,yard,community_events,sidewalks,walk_night,five_years,kids_outside,car,restaurants,streets,holiday,quiet,wildlife\n')
...
writeReviews.write('review_id,user_id,neighborhood_id,username,user_type,review_date,review_text,likes,community,commute,dog_owner,parent\n', 'utf-8')



const writeNumNeighByListingId = (numListings, numNeighborhoods, writer, encoding, callback) => {
  let i = numListings;
  let id = 0;
  const write = () => {
    let ok = true;
    do {
      ...
      let neighborhood = `${id},${neighborhood_id},'${name}',${dog_friendly},${grocery_stores},${neighbors_friendly},${parking_easy},${yard},${community_events},${sidewalks},${walk_night},${five_years},${kids_outside},${car},${restaurants},${streets},${holiday},${quiet},${wildlife}\n`
      if (i === 0) {
...
const writeNumReviews = (numReviews, numUsers, numNeighborhoods, writer, encoding, callback) => {
  ...
  const write = () => {
    ...
    do {
      ...
      const user  = `${id},${user_id},${neighborhood_id},${username},${user_type},${review_date},${full_text},${likes},${community},${commute},${dog_owner},${parent}\n`;
      if (i === 0) {
...
writeNumNeighByListingId(10000000, 10000000, writeNeighborhoods, 'utf-8', (err, data) => {
  if (err) {
    console.log(err);
  } else {
    var now = new Date();
...
writeNumReviews(100000000, 5000000, 10000000, writeReviews, 'utf-8', (err, data) => {
  if (err) {
    console.log(err);
  } else {
    var now = new Date();
